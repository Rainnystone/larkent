...
- **backfill**: the idempotent catch-up routine that runs on a recovery signal: pull recent history with **bot identity** for listed groups and **session-known p2p** chats, keep survivors that are not in the processed ledger, and hand them to the normal `intakeMessage` path. Groups still require an @-mention of this bot. p2p keeps ordinary human messages (no @ required). `maxMentionsPerChat` is the newest-first enqueue cap per chat for both (group @mentions and p2p human messages); the preference key is not renamed. Not a poller; it never runs on a timer.
- **session-known p2p**: a DM chat id already present as a session / catalog scope that `getChatMode` resolves as `'p2p'`. Backfill unions those ids with `listChats` groups. Unique session chat ids are classified concurrently, unlisted first, and bounded by `maxChats`. `maxChats` applies to that combined list and preserves discovered p2p ids within the cap. A first-contact DM with no prior session entry is out of scope.
...
- **incomplete scan**: a backfill that fetched some chats and failed at least one, or that could not classify a session chat's mode. It does not advance `lastBackfillEnd`. The ledger keeps `incompleteFrom` so the next recovery retries that window even if keepalive has already moved `lastLiveAt`.
