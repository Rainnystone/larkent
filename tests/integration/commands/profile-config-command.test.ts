  ...
  const root: RootConfig = {
    ...
    profiles: {
      claude: createDefaultProfileConfig({
        agentKind: 'claude',
        // These tests drive lark-cli identity switches; pin personal mode
        // explicitly (the fork defaults to team), and start from bot-only so
        // switching to user-default is a real change (fork lark-cli default
        // is user-default now).
        mode: 'personal',
