...
describe('markdown stream startup failures', () => {
  ...
  it('kimi final-only round sends exactly one final reply, no progress stream', async () => {
    // Kimi's `-p` stream-json emits the answer only as a terminal `final_text`
    // (no text deltas), so nothing ever opens the lazy progress stream. The
    // regression this guards: the streaming fallback measured emptiness with
    // renderText, which ignores finalText — the reply silently went nowhere.
    ...
    await h.channel.handlers.message?.(message('om_kimi_final', 'run'));
    await waitFor(() => h.channel.sent.length === 1);
...
async function createHarness(options: {
  ...
  /** Codex/kimi hold the answer back for a dedicated final reply; Claude streams it. */
  agentKind?: 'claude' | 'codex' | 'kimi';
} = {}): Promise<{
  tmp: TmpProfile;
