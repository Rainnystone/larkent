...
describe('profile schema', () => {
  it('defaults Claude sandbox to danger-full-access through canonical permissions', () => {
    ...
    expect(cfg.agent).toEqual({ kind: 'claude' });
  ...
  it('defaults deployment mode to team and parses personal', () => {
    const fresh = createDefaultProfileConfig({ agentKind: 'claude', accounts: { app } });
    expect(fresh.mode).toBe('team');

    const personal = createDefaultProfileConfig({ agentKind: 'claude', mode: 'personal', accounts: { app } });
    expect(personal.mode).toBe('personal');

    // Unknown / missing values normalize to team (this fork's open default).
    ...
    expect(bogus.mode).toBe('team');
  });

  it('effectiveLarkCliIdentity passes the stored preset through in both modes', () => {
    // Fork behavior: team mode no longer forces bot-only.
    expect(
  ...
  it('defaults lark-cli identity to user-default without legacy global source fields', () => {
    const cfg = createDefaultProfileConfig({
      agentKind: 'claude',
      accounts: { app },
    });

    // Fork default: user-default (bot identity remains the no-auth fallback).
    expect(cfg.larkCli).toEqual({ identityPreset: 'user-default' });
