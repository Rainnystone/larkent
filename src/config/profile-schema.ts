 ...
 *   - `personal`: only owner + allowlisted users/chats can use the bot.
 *   - `team` (default in this fork): anyone can @-use the bot (no allowlist
 *     gating).
 *
 * Upstream also bound lark-cli identity to this switch (team ⇒ bot-only).
 * This fork decouples them: see {@link effectiveLarkCliIdentity}.
...
/**
 * The lark-cli identity preset that takes effect. Upstream forced `bot-only`
 * in team mode; this fork deliberately decouples the two: team mode opens
 * *usage* to everyone, while the stored identityPreset governs which lark-cli
 * identity the agent wields. With the fork's `user-default` default, an
 * owner-completed OAuth login lets the agent read chat history and edit
 * tenant assets on behalf of the owner — including in team mode.
 */
export function effectiveLarkCliIdentity(
  profile: Pick<ProfileConfig, 'mode' | 'larkCli'>,
): LarkCliIdentityPreset {
  return profile.larkCli.identityPreset;
}
...
function normalizeLarkCli(input: unknown): LarkCliConfig {
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    // Fork default: user-default so a first-run profile can act with the
    // owner's authorized user identity once OAuth completes (bot identity
    // remains the fallback when no user auth exists).
    return { identityPreset: 'user-default' };
  }
  const raw = input as {
  ...
  const identityPreset: LarkCliIdentityPreset =
    raw.identityPreset === 'bot-only' ? 'bot-only' : 'user-default';
