...
async function applyResume(nonce: string, ctx: CommandContext): Promise<void> {
  if (ctx.sessionCatalog && ctx.sessionCatalogIdentity) {
    const entry = ctx.sessionCatalog.activeFor(ctx.sessionCatalogIdentity);
    const resolved = consumeResumeCandidate(nonce, ctx.sessionCatalogIdentity);
    if (resolved) {
      ctx.activeRuns.interrupt(ctx.scope);
      ...
      if (descriptorFor(ctx.sessionCatalogIdentity.agentId).resumeHistory !== 'codex-thread') {
    ...
    ctx.activeRuns.interrupt(ctx.scope);
    ctx.sessions.set(ctx.scope, nonce, ctx.sessionCatalogIdentity.cwdRealpath);
    await reply(ctx, RESUME_APPLIED_REPLY);
    return;
  }
  ...
  ctx.activeRuns.interrupt(ctx.scope);
  ctx.sessions.set(ctx.scope, nonce, cwd);
  await reply(ctx, RESUME_APPLIED_REPLY);
}
...
async function larkCliStatus(ctx: CommandContext): Promise<'app' | 'user-ready' | 'user-missing' | 'check-failed'> {
  ...
  try {
    ...
    // Fork: user-default policy = strict off + default-as bot (user identity
    // via explicit `--as user`), so accept bot as the ready default too.
    if (
      (app?.defaultAs === 'auto' || app?.defaultAs === 'bot') &&
      app.strictMode === 'off' &&
...
async function handleStatus(_args: string, ctx: CommandContext): Promise<void> {
  ...
  const card = statusCard({
    ...
    cwd,
    sessionId: isCodex ? catalogEntry?.resumeHandle : sess?.sessionId,
...
async function submitConfig(ctx: CommandContext): Promise<void> {
  ...
  // Effective preset = what actually gets applied to lark-cli. This fork
  // decouples deployment mode from identity: the select value applies as-is
  // in both personal and team mode. Re-apply the lark-cli policy whenever the
  // *effective* preset changes.
  const nextEffectiveIdentity: LarkCliIdentityPreset = larkCliIdentity;
