...
import { agentKindHelpList } from '../agent/registry';
...
program
  ...
  .option('--agent <kind>', `agent kind for a new profile (${agentKindHelpList()})`)
...
program
  ...
  .option('--agent <kind>', `agent kind for legacy v1 profile migration (${agentKindHelpList()})`)
...
profile
  ...
  .option('--agent <kind>', `agent kind (${agentKindHelpList()})`)
...
program
  ...
  .option('--agent <kind>', `agent kind for first-run profile bootstrap (${agentKindHelpList()})`)
  .option('--workspace <path>', 'initial working directory for first-run profile bootstrap')
  .option('--app-id <id>', 'use an existing Lark/Feishu app instead of QR app creation')
  .option('--app-secret <secret>', 'App Secret for --app-id; prefer interactive input on shared machines')
  .option('--tenant <tenant>', 'tenant for --app-id (feishu or lark; default feishu)')
