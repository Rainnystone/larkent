...
import { agentKindChoices } from '../agent/registry';
...
program
  ...
  .option('--agent <kind>', `agent kind for a new profile (${agentKindChoices()})`)
...
program
  ...
  .option('--agent <kind>', `agent kind for legacy v1 profile migration (${agentKindChoices()})`)
...
profile
  ...
  .option('--agent <kind>', `agent kind (${agentKindChoices()})`)
...
program
  ...
  .option('--agent <kind>', `agent kind for first-run profile bootstrap (${agentKindChoices()})`)
  .option('--workspace <path>', 'initial working directory for first-run profile bootstrap')
  .option('--app-id <id>', 'use an existing Lark/Feishu app instead of QR app creation')
  .option('--app-secret <secret>', 'App Secret for --app-id; prefer interactive input on shared machines')
  .option('--tenant <tenant>', 'tenant for --app-id (feishu or lark; default feishu)')
