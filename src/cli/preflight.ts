...
async function checkLarkCli(opts: PreFlightOptions): Promise<void> {
  ...
  if (privateBinding) {
    // Fork note: upstream forced team-mode profiles to bot-only here. This
    // fork decouples deployment mode from lark-cli identity, so team
    // profiles follow the same import/switch flow as personal ones.
...
async function switchLarkCliIdentityPolicy(
  ...
  // Fork: bot stays the default identity even for user-default; `--as user`
  // is the explicit opt-in for user-scoped reads (chat history, docs).
  const defaultAs = 'bot';
