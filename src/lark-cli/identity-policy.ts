...
export async function applyLarkCliIdentityPolicy(
  ...
  // Fork: user-default keeps `bot` as the default identity (upstream used
  // `auto`, which prefers the user). Chat stays the bot; the agent opts into
  // the user identity per call with `--as user` for history/docs.
  const defaultAs = 'bot';
